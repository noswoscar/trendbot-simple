...
if [ -t 1 ] && [ "$(tput colors 2>/dev/null)" ]; then
    RED=$(tput setaf 1)
    GREEN=$(tput setaf 2)
    YELLOW=$(tput setaf 3)
    BLUE=$(tput setaf 4)
...
test_bot() {
    ...
    if [[ -n "$win_rate" ]] && [[ "$win_rate" != "null" ]]; then
...
print_summary() {
    ...
    if [[ "$support" != "N/A" ]] && [[ "$support" != "null" ]] && [[ "$current_price" != "N/A" ]]; then
        ...
        if [[ $dist_sup -lt 100 ]] 2>/dev/null; then
            echo "    ⚠️ Price is near support (${dist_sup} pts) - watch for breakdown"
        fi
    fi
    ...
    if [[ "$resistance" != "N/A" ]] && [[ "$resistance" != "null" ]] && [[ "$current_price" != "N/A" ]]; then
        ...
        if [[ $dist_res -lt 100 ]] 2>/dev/null; then
            echo "    ⚠️ Price is near resistance (${dist_res} pts) - watch for breakout"
        fi
    fi

    if [[ "$bot_confidence" != "0" ]] && (( $(echo "$bot_confidence > 0.2" | bc -l 2>/dev/null) )); then
        echo "    ✅ Bot confidence is high (${bot_confidence}) - ready to trade!"
    else
        echo "    ⏳ Bot waiting for signal (confidence: ${bot_confidence})"
    fi

    local trades=$(curl -s "${BOT_URL}/api/bot/history" | jq -r '.stats.totalTrades // 0')
    if [[ $trades -gt 0 ]]; then
        echo "    ✅ Bot has made ${trades} trades"
    else
        echo "    ⏳ No trades yet - waiting for setup"
    fi
    echo ""
}

...
test_levels() {
    ...
    if [[ "$support" != "N/A" ]] && [[ "$support" != "null" ]]; then
        ...
        printf "    Support: %s (%.0f pts below) strength: %s\n" "$support" "$dist_sup" "$support_strength"
    else
    ...
    if [[ "$resistance" != "N/A" ]] && [[ "$resistance" != "null" ]]; then
        ...
        printf "    Resistance: %s (%.0f pts above) strength: %s\n" "$resistance" "$dist_res" "$resistance_strength"
    else
